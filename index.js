...
//２日分の日付を表示
document.getElementById("today").textContent = day.getMonth()+1 + "/" + day.getDate() + "(" + weekDay[day.getDay()] + ")" ;
document.getElementById("tomorrow").textContent = tomorrow.getMonth()+1 + "/" + tomorrow.getDate() + "(" + weekDay[tomorrow.getDay()] + ")" ;

// //読み込み時即実行
// (function () {
//     //データを取得する先のurlを作成後半にある数字の部分を変えると様々な県の気象データが取得できる
//     //茨城だと080000　東京だと130000　北海道だと016000
...
//         console.log(weather)
//         document.getElementById("title").textContent = weather[1].timeSeries[0].areas[0].area.name + "の天気";

//         //今日と明日の気象データ(weatherCodes)を抽出
//         const td_w = weather[0].timeSeries[0].areas[0].weatherCodes[0];
//         const tm_w = weather[0].timeSeries[0].areas[0].weatherCodes[1];

//         //今日の最高気温と最低気温を抽出
//         const td_t_h = weather[1].tempAverage.areas[0].max;
//         const td_t_l = weather[1].tempAverage.areas[0].min;

//         //降水確率データの個数を取得（時間帯によって個数は変動）
//         let pops_num = weather[0].timeSeries[1].areas[0].pops.length - 1;

//         //抽出したデータをもとに画面上に表示
//         document.getElementById("td_w").textContent = weathers[td_w][3]; 
//         document.getElementById("td_w_img").src = "https://www.jma.go.jp/bosai/forecast/img/" + weathers[td_w][0];
//         document.getElementById("td_t_h").textContent = td_t_h;
//         document.getElementById("td_t_l").textContent = td_t_l; 

//         document.getElementById("tm_w").textContent = weathers[tm_w][3]; 
//         document.getElementById("tm_w_img").src = "https://www.jma.go.jp/bosai/forecast/img/"+weathers[tm_w][0];

//         for(i = 8; pops_num >= 0; i--){
//             document.getElementById(`${i}`).textContent = weather[0].timeSeries[1].areas[0].pops[pops_num];
//             pops_num--;
//         }
...
function weatherInfo (prefNum) {
    ...
    // データを取得する先のurlを作成
    const url = `https://www.jma.go.jp/bosai/forecast/data/forecast/${fileName}.json`;
